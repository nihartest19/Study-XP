...
import { Plus, Trash2, Star, ShieldAlert, Award, Zap } from "lucide-react";
...
// ── Category config ───────────────────────────────────────────────────────────
// XP values mirror the backend exactly — shown for transparency, not editable.
const CATEGORIES = [
  { value: "daily_revision",    label: "Daily Habit / Revision",  xp: 10,  color: "border-l-blue-400" },
  { value: "study_session",     label: "Study Session",           xp: 20,  color: "border-l-primary" },
  { value: "assignment",        label: "Assignment / Homework",   xp: 35,  color: "border-l-amber-500" },
  { value: "quiz",              label: "Quiz / Test Practice",    xp: 40,  color: "border-l-orange-500" },
  { value: "project_milestone", label: "Project Milestone",       xp: 50,  color: "border-l-secondary" },
] as const;
...
const taskSchema = z.object({
  ...
  subjectId: z.string().optional().transform((v) => (v && v !== "none" ? parseInt(v) : undefined)),
});
...
export default function Tasks() {
  ...
  const selectedMeta = getCategoryMeta(watchedCategory);

  const onSubmit = (data: z.infer<typeof taskSchema>) => {
    createTask.mutate(
      { data: { title: data.title, category: data.category, subjectId: data.subjectId ?? null } },
      {
        onSuccess: () => {
        ...
        onError: (err: unknown) => {
          const msg =
            ...
            "Could not create quest. Try again.";
        ...
        onSuccess: (result) => {
          ...
          if (completed) {
            ...
            if (result.xpAwarded > 0) {
              ...
              toast({
                title: "Quest Completed!",
                description: "Daily XP cap reached — rest and come back tomorrow!",
              });
            }
          ...
          <DialogContent>
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                {/* Category — XP is set by backend based on this */}
                <FormField
                  control={form.control}
                  name="category"
                  render={({ field }) => (
      ...
      {tasksLoading ? (
        ...
        <div className="text-center p-12 border border-dashed rounded-xl bg-muted/10">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Enjoy your rest, hero. Or create a new quest!
          </p>
        </div>
      ) : (
                      ...
                      <div className="flex items-center gap-4 w-full sm:w-auto flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-2 mt-1">
                            ...
                            <Badge
                              variant="outline"
                              className="text-xs font-normal px-2 py-0 h-5 bg-muted/30"
                            >
                              {meta.label}
                            </Badge>
                            {/* Subject badge */}
                            {task.subjectName && (
                              <Badge
                                variant="outline"
                                className="text-xs bg-muted/30 font-normal px-2 py-0 h-5"
                                style={
                                  task.subjectColor
                                    ? { borderColor: task.subjectColor, color: task.subjectColor }
                                    : {}
                                }
                              >
                                {task.subjectName}
